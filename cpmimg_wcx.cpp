...
extern "C" {
	...
	DLLEXPORT archive_HANDLE STDCALL OpenArchive(tOpenArchiveData* ArchiveData)
	{
		...
		const char* format = "osbexec1"; //  osb1sssd, osbexec1
		// struct cpmInode root;
		const char* driver_name = nullptr; // devopts; example: driver_name=="imd", "tele" etc.
		int uppercase = 1;
		const char* errs = Device_open(&super.dev, ArchiveData->ArcName, O_RDONLY, driver_name);
		...
		int erri = cpmReadSuper(&super, &root, format, uppercase);
		if (erri == -1) 
		{
